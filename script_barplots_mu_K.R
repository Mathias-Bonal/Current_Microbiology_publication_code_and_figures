...
#
# Create the barplots comparing mu and K values as presented in Bonal et al, 2022
#
...
#### Create barplots for growth rates (Fig. 5a) ####
...
plot_78R_mu <- ggbarplot(s78R, x="culture", y="mu", xlab=F, ylab=F, ylim=c(0,0.8), color="darkorchid", fill="darkorchid") +
  ...
  # set significance arcs (only significant arcs are plotted, i.e. unpaired Student test giving a significative
...
plot_515Z_K <- ggbarplot(s515Z, x="culture", y="K", xlab=F, ylab=F, ylim=c(0,25), color="darkgreen", fill="darkgreen") +
  geom_errorbar(aes(x=culture, ymin=K-stdev.K, ymax=K+stdev.K), width=0.1, colour="black", alpha=0.7, size=0.5) +
  geom_signif(y_position=23.25, xmin=1.05, xmax=3.1, annotation="***", tip_length=0.05) +
  ...
  geom_signif(y_position=21.75, xmin=2.05, xmax=3.95, annotation="**", tip_length=0.05) +
...
plot_1315Z_K <- ggbarplot(s1315Z, x="culture", y="K", xlab=F, ylab=F, ylim=c(0,25), color="gray47", fill="gray47") + 
  geom_errorbar(aes(x=culture, ymin=K-stdev.K, ymax=K+stdev.K), width=0.1, colour="black", alpha=0.7, size=0.5) +
  geom_signif(y_position=21.5, xmin=1.05, xmax=2.95, annotation="**", tip_length=0.05) +
  ...
  geom_signif(y_position=19.5, xmin=2.1, xmax=2.90, annotation="*", tip_length=0.05) +
  geom_signif(comparisons=c("bi", "quadri"), y_position=19.5, tip_length=0.05, vjust=0.2) +
  theme(axis.text.x = element_text(size=14), axis.text.y = element_text(size=14)) +
  scale_x_discrete(labels=labels_barplots)
